...
void test_free_functions() {
    ...
    fc::Vector2 perp = fc::perpendicular(p);
    assert(perp.x() == -2.0f);
    assert(perp.y() == 1.0f);

    fc::Vector2 z = fc::vector2_zero();
    assert(z.x() == 0.0f && z.y() == 0.0f);

    fc::Vector2 ux = fc::vector2_unit_x();
    assert(ux.x() == 1.0f && ux.y() == 0.0f);

    fc::Vector2 uy = fc::vector2_unit_y();
