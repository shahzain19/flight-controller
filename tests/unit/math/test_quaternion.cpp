...
void test_identity() {
    [[maybe_unused]] fc::Quaternion q;
    assert(q.x() == 0.0f);
    assert(q.y() == 0.0f);
    assert(q.z() == 0.0f);
    assert(q.w() == 1.0f);

    [[maybe_unused]] fc::Quaternion q2 = fc::quaternion_identity();
    assert(q == q2);
...
void test_from_axis_angle_zero() {
    [[maybe_unused]] fc::Quaternion q = fc::Quaternion::from_axis_angle(
...
void test_from_axis_angle_90z() {
    ...
    [[maybe_unused]] float s2 = static_cast<float>(M_SQRT2) / 2.0f;
    assert(approx_eq(q.x(), 0.0f));
    assert(approx_eq(q.y(), 0.0f));
...
void test_from_euler_zero() {
    [[maybe_unused]] fc::Quaternion q = fc::Quaternion::from_euler(0.0f, 0.0f, 0.0f);
    assert(approx_eq(q.x(), 0.0f));
    assert(approx_eq(q.y(), 0.0f));
    assert(approx_eq(q.z(), 0.0f));
    assert(approx_eq(q.w(), 1.0f));
...
void test_from_to_matrix_identity() {
    ...
    [[maybe_unused]] fc::Quaternion q = fc::Quaternion::from_matrix(m);
    assert(approx_eq(q.x(), 0.0f));
    assert(approx_eq(q.y(), 0.0f));
    assert(approx_eq(q.z(), 0.0f));
    assert(approx_eq(q.w(), 1.0f));
...
void test_to_matrix3_roundtrip() {
    ...
    [[maybe_unused]] fc::Quaternion diff = orig * back.conjugate();
...
void test_conjugate() {
    fc::Quaternion q(1.0f, 2.0f, 3.0f, 4.0f);
    [[maybe_unused]] fc::Quaternion c = q.conjugate();
    assert(c.x() == -1.0f);
    assert(c.y() == -2.0f);
    assert(c.z() == -3.0f);
    assert(c.w() == 4.0f);
...
void test_inverse() {
    ...
    [[maybe_unused]] fc::Quaternion product = q * inv;
...
void test_normalize() {
    fc::Quaternion q(1.0f, 2.0f, 3.0f, 4.0f);
    [[maybe_unused]] fc::Quaternion n = q.normalized();
...
void test_multiply_identity() {
    ...
    fc::Quaternion q(1.0f, 2.0f, 3.0f, 4.0f);
    [[maybe_unused]] fc::Quaternion r = q * id;
    assert(r == q);

    [[maybe_unused]] fc::Quaternion r2 = id * q;
    assert(r2 == q);
...
void test_rotate_zero_angle() {
    ...
    [[maybe_unused]] fc::Vector3 r = id.rotate(v);
...
void test_rotate_90z() {
    ...
    [[maybe_unused]] fc::Vector3 r = q.rotate(v);
...
void test_free_functions() {
    ...
    [[maybe_unused]] fc::Quaternion c1 = fc::conjugate(q);
    fc::Quaternion c2 = q.conjugate();
    assert(c1 == c2);

    [[maybe_unused]] fc::Quaternion i1 = fc::inverse(q);
    ...
    [[maybe_unused]] float d = fc::dot(q, q);
...
void test_data_access() {
    fc::Quaternion q(1.0f, 2.0f, 3.0f, 4.0f);
    [[maybe_unused]] const float* d = q.data();
    assert(d[0] == 1.0f);
    assert(d[1] == 2.0f);
    assert(d[2] == 3.0f);
    assert(d[3] == 4.0f);
